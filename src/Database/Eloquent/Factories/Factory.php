...
use SoftTent\WpEloquent\Database\Eloquent\Collection as EloquentCollection;
...
abstract class Factory
{
     ...
     * @param  \SoftTent\WpEloquent\Support\Collection  $states
     * @param  \SoftTent\WpEloquent\Support\Collection  $has
     * @param  \SoftTent\WpEloquent\Support\Collection  $for
     * @param  \SoftTent\WpEloquent\Support\Collection  $afterMaking
     * @param  \SoftTent\WpEloquent\Support\Collection  $afterCreating
     ...
     * @param  \SoftTent\WpEloquent\Database\Eloquent\Model|null  $parent
     * @return array
     */
     ...
     * @return \SoftTent\WpEloquent\Database\Eloquent\Collection|mixed
     */
     ...
     * @param  array  $attributes
     * @param  \SoftTent\WpEloquent\Database\Eloquent\Model|null  $parent
     * @return \SoftTent\WpEloquent\Database\Eloquent\Collection|\SoftTent\WpEloquent\Database\Eloquent\Model|mixed
     */
    public function create($attributes = [], ?Model $parent = null)
    {
        ...
        if ($results instanceof Model) {
            $this->store(asdb_collect([$results]));

            $this->callAfterCreating(asdb_collect([$results]), $parent);
        } else {
            $this->store($results);
     ...
     *
     * @param  \SoftTent\WpEloquent\Support\Collection  $results
     * @return void
     */
    ...
    public function make($attributes = [], ?Model $parent = null)
    {
        ...
        if ($this->count === null) {
            return asdb_tap($this->makeInstance($parent), function ($instance) {
                $this->callAfterMaking(asdb_collect([$instance]));
            });
        }
    ...
    protected function makeInstance(?Model $parent)
    {
        return Model::unguarded(function () use ($parent) {
            return asdb_tap($this->newModel($this->getExpandedAttributes($parent)), function ($instance) {
                if (isset($this->connection)) {
    ...
    {
        return asdb_collect($definition)->map(function ($attribute, $key) use (&$definition) {
    ...
    {
        $guess = Str::camel(Str::plural(asdb_class_basename($related)));
    ...
    public function hasAttached(self $factory, $pivot = [], $relationship = null)
    {
        return $this->newInstance([
            'has' => $this->has->concat([new BelongsToManyRelationship(
                $factory,
                $pivot,
                $relationship ?: Str::camel(Str::plural(asdb_class_basename($factory->modelName())))
            )]),
        ]);
    }
    ...
    public function for(self $factory, $relationship = null)
    {
        return $this->newInstance(['for' => $this->for->concat([new BelongsToRelationship(
            $factory,
            $relationship ?: Str::camel(asdb_class_basename($factory->modelName()))
        )])]);
    }
     ...
     *
     * @param  \SoftTent\WpEloquent\Support\Collection  $instances
     * @return void
     */
    ...
    public function modelName()
    {
        $resolver = static::$modelNameResolver ?: function (self $factory) {
            $factoryBasename = Str::replaceLast('Factory', '', asdb_class_basename($factory));
