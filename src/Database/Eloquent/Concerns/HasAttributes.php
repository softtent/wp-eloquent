...
use SoftTent\WpEloquent\Contracts\Database\Eloquent\Castable;
use SoftTent\WpEloquent\Contracts\Database\Eloquent\CastsInboundAttributes;
use SoftTent\WpEloquent\Contracts\Support\Arrayable;
use SoftTent\WpEloquent\Database\Eloquent\InvalidCastException;
use SoftTent\WpEloquent\Database\Eloquent\JsonEncodingException;
use SoftTent\WpEloquent\Database\Eloquent\Relations\Relation;
use SoftTent\WpEloquent\Support\Arr;
use SoftTent\WpEloquent\Support\Carbon;
use SoftTent\WpEloquent\Support\Collection as BaseCollection;
use SoftTent\WpEloquent\Support\Facades\Date;
...
trait HasAttributes
{
    ...
    protected function getRelationshipFromMethod($method)
    {
        ...
        return asdb_tap($relation->getResults(), function ($results) use ($method) {
    ...
    {
        return asdb_tap($this->getArrayAttributeByKey($key), function (&$array) use ($path, $value) {
     ...
     *
     * @param  mixed  $value
     * @return \SoftTent\WpEloquent\Support\Carbon
     */
    ...
    {
        // Assuming $yourDateTime is your DateTime object
        $yourDateTime = new \DateTime(); // For the current date and time

        // Format the DateTime object to a storable string
        $storableString = $yourDateTime->format('Y-m-d H:i:s');
     ...
     * @return bool
     *
     * @throws \SoftTent\WpEloquent\Database\Eloquent\InvalidCastException
     */
    ...
    protected function getOriginalWithoutRewindingModel($key = null, $default = null)
    {
        ...
        return asdb_collect($this->original)->mapWithKeys(function ($value, $key) {
    ...
    {
        static::$mutatorCache[$class] = asdb_collect(static::getMutatorMethods($class))->map(function ($match) {
